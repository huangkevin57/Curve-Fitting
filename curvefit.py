...
for i in range(len(lines)):
	...
	if diff < .15 * screen_height: # or maximum < 2 * screen_height / 3.0 or len(lines[i]) < 100:
		...
		# 	curve_line(res, i, evaluation(p, i))
		# h0 = points_to_fit_h[0]
		# w0 = points_to_fit_w[0]
		# h1 = max(0, (int) (points_to_fit_h[0] - .12 * screen_height))
		# w1 = (int) (evaluation(p, h1))

		# slope = 1.0 * (w0 - w1) / (h0 - h1)
		# intercept = 1.0 * (w1 * h0 - h1 * w0) / (h0 - h1)
		# for k in range(0, h0 + 1):
		# 	tangent_line(res, k, slope * k + intercept)


		# slope = 1.0 * (points_to_fit_w[0] - tangent_point_w) / (points_to_fit_h[0] - tangent_point_h)
		# intercept = 1.0 * (tangent_point_w * points_to_fit_h[0] - tangent_point_h * points_to_fit_w[0]) \
		# 	/ (points_to_fit_h[0] - tangent_point_h)

		# for k in range(tangent_point_h, points_to_fit_h[0] + 1):
		# 	curve_line(res, k, slope * k + intercept)


		# h_current = tangent_point_h
		# w_current = tangent_point_w
		# diff = points_to_fit_h[0] - tangent_point_h
		# slope = 1.0 * (points_to_fit_w[0] - tangent_point_w) / (points_to_fit_h[0] - tangent_point_h)
		# for k in range(diff + 1):
		# 	h_current += 1
		# 	w_current += 
		# curve_line(res, tangent_point_h, tangent_point_w)
			#print p(i)
